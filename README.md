...
Temporary failures, such as rate limiting or a brief outage, are retried
a few times with backoff, honoring LegiScan’s `Retry-After` header.
Other failures stop right away with a classed error you can catch, such
as `legihelpR_rate_limited`, `legihelpR_quota_exceeded`,
`legihelpR_invalid_key`, or `legihelpR_invalid_request`. All of them
inherit from `legihelpR_error`. See `?"legihelpR-errors"` for the full
list.
...
If LegiScan asks for a wait longer than 60 seconds, legihelpR stops
instead of waiting. Change the limit with
`options(legihelpR.max_retry_wait = <seconds>)`.
